    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        el.draggable(_.extend(this.opts.draggable, {
            ...
            drag: function (event, ui) {
                ...
                var gridStack = self, $gridStackEl;
                var grid = self.grid;

                if( $(this).hasClass('grid-stack-subcol') ) {
                    var $overRow = $('.ui-state-hover');
                    if( $overRow.length ) {
                        $gridStackEl = $overRow.find('.grid-stack');
                        gridStack = $gridStackEl.data('gridstack');
                        grid = gridStack.grid;
                        y = Math.floor((ui.position.top - $overRow.position().top - $gridStackEl.position().top + cell_height/2) / cell_height);
                    } else {
                        $gridStackEl = $('#catalog-holder > .grid-stack');
                        gridStack = $gridStackEl.data('gridstack');
                        grid = gridStack.grid;
                        y = Math.floor((ui.position.top + cell_height/2) / cell_height);
                    }

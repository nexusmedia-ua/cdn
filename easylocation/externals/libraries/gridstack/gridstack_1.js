...
(function (factory) {
    ...
    var GridStack = function (el, opts) {
        ...
        var oldHeight = this.grid.get_grid_height() * (this.opts.cell_height + this.opts.vertical_margin) - this.opts.vertical_margin;
        var newHeight = this.grid.get_grid_height() *  this.opts.cell_height + (this.grid.nodes.length > 0 ? this.grid.nodes.length - 1 : 0) * this.opts.vertical_margin;
        ...
        if (max_height > this._styles._max) {
            for (var i = this._styles._max; i < max_height; ++i) {
                Utils.insert_css_rule(this._styles,
                    ...
                    'height: ' + (this.opts.cell_height * (i + 1) + this.opts.vertical_margin ) + 'px;',
                    i
                );
                Utils.insert_css_rule(this._styles,
                    prefix + '[data-gs-min-height="' + (i + 1) + '"]',
                    'min-height: ' + (this.opts.cell_height * (i + 1) ) + 'px;',
                    i
                );
                Utils.insert_css_rule(this._styles,
                    prefix + '[data-gs-max-height="' + (i + 1) + '"]',
                    'max-height: ' + (this.opts.cell_height * (i + 1) + this.opts.vertical_margin ) + 'px;',
                    i
                );
                /*Utils.insert_css_rule(this._styles,
                    prefix + '[data-gs-y="' + i + '"]',
                    'top: ' + (this.opts.cell_height * i + i * this.opts.vertical_margin ) + 'px;',
                    i
                );*/


                var totalHeight = 0;
                var $els = $(prefix).slice(1, i);
                $els.each(function(){
                  totalHeight += +($(this).attr('data-gs-height') ? $(this).attr('data-gs-height') : 0);
                  console.log($(this).find('.grid-stack-subcol').length);
                })
                totalHeight = this.opts.cell_height * totalHeight + $els.length * this.opts.vertical_margin;

                Utils.insert_css_rule(this._styles,
                    prefix + ':nth-child(' + (i + 1) + ')',
                    'top: ' + totalHeight + 'px;',
                    i
                );
    ...
    GridStack.prototype._prepare_element = function (el) {
        ...
        var on_end_moving = function (event, ui) {
            ...
            // no move
            if( o.attr('data-gs-x') == node.x && o.attr('data-gs-y') == node.y && o.attr('data-gs-width') == node.width && o.attr('data-gs-height') == node.height) {
                o.removeAttr('style');
                // hack for disabling click event
                ajaxIsActive = true;
                setTimeout(function(){ ajaxIsActive = false; }, 50);
            } else {
                o
        ...
        el.draggable(_.extend(this.opts.draggable, {
            ...
            drag: function (event, ui) {
                ...
                //if( self.opts.fixedPlaceholder ) y = node.y;

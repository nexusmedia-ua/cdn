    ...
    function easyVideoSetup() {
      ...
      ev$("img[alt*='youtu.be/']").click(function(){
        if (!ev$(this).is(ev$( ev_main_img_selector ))) {
          ...
          ev$('#easyvideo_video').css("line-height","0").css("position","relative").css("z-index", "9999").html('<iframe width="100%"  src="https://www.youtube.com/embed/' + ev$(this).prop('alt').replace("https://youtu.be/", "") + '" frameborder="0" allowfullscreen></iframe>').find("iframe").attr("height", ev$("#easyvideo_video").width() * 0.5625);
      ...
      ev$("img[alt*='youtube.com/watch']").click(function(){
        if (!ev$(this).is(ev$( ev_main_img_selector ))) {
          ...
          ev_main_img.hide();
          ev$('.MagicZoomPlus figure > img').hide();
          ev$('#easyvideo_video').css("line-height","0").css("position","relative").css("z-index", "9999").html('<iframe width="100%"  src="https://www.youtube.com/embed/' + ev$(this).prop('alt').replace("https://www.youtube.com/watch?v=", "") + '" frameborder="0" allowfullscreen></iframe>').find("iframe").attr("height", ev$("#easyvideo_video").width() * 0.5625);
          setTimeout(function(){ if (is_zoom) ev_container.removeClass(zoom_class); ev$('#easyvideo_video').find("iframe").attr("height", ev$("#easyvideo_video").width() * 0.5625); }, 50);
          ev$('#easyvideo_video').show();
        }
      });

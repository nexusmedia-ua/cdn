...
class ShopifyClient {
  ...
  public function createCharge($price, $title, $return_url, $test_status, $trial_days = 0)
  {
		$charge = array
		(
		...
		if (!$test_status) {
			unset($charge['recurring_application_charge']['test']);
		}

		try {
			return $recurring_application_charge = $this->call('POST', '/admin/recurring_application_charges.json', $charge);
		} catch (ShopifyApiException $e) {
			echo "Shopify error: ".$e->getMessage();
			die();
		}
  }

	// create a new recurring charge
  public function createOnetimeCharge($price, $title, $return_url, $test_status)
  {
		$charge = array
		(
		...
		if (!$test_status) {
			unset($charge['application_charge']['test']);
		}

		try {
			$recurring_application_charge = $this->call('POST', '/admin/application_charges.json', $charge);
	...
	public function getAccessToken($code) {
		...
		$query = "client_id={$this->api_key}&client_secret={$this->secret}&code=$code";
		$response = $this->curlHttpApiRequest('POST', $url, '', $query, array());
...
if (!class_exists("ShopifyCurlException")) {
	class ShopifyCurlException extends Exception {
		...
		function getErrorCode() { return $this->errno; }

	}
}

if (!class_exists("ShopifyApiException")) {

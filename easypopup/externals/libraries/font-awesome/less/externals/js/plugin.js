...
function Controller(widgets)
{
  ...
  this.edit_content = function(item, event) {
    ...
    var contentId = $editingElement.attr('data-content-id');
    var contentType = $editingElement.attr('data-content-type');

    if( contentType == 'column' ) {
      showWidgetsMenu(event);
    } else {
      editWidgetContent(item, event);
    }
  }
      ...
      createViewModel: function (controller, componentInfo) {
        var ViewModel = function (controller, componentInfo) {
          ...
          this.afterAddWidget = function (items) {
            ...
            var $item = $(item);
            var isColumn = !$item.parent().closest('.layout-column').length;
            var cellHeight = isColumn ? 168 : 60;
              ...
              if( item.getAttribute('data-content-id') == '0' ) {
                ...
                if( request ) {
                  request.done(function(response) {
                    ...
                    if( response.preview ) updateQuickPreviewContent( response.preview );
                  });
                }
              } else {
...
function showWidgetsMenu( event )
{
  var $column = $(event.target).parent().closest('.layout-column');
...
function editWidgetContent(params, event)
{
  ...
  $editingElement = $(event.target).parent().closest('.layout-widget');
    ...
    if( request ) {
      request.done(function(response) {
        ...
        if( !response.error ) {
          ...
          if( response.preview ) updateQuickPreviewContent( response.preview );
          if( isset(ShopifyApp) ) ShopifyApp.Modal.alert("Widget content saved");
        } else {
      ...
      request.done(function(response) {
        ...
        if( !response.error ) {
          ...
          if( response.data.status == 1 ) {
            $('#layout-item-' + layoutId + ' .layout-item-status').removeClass('layout-disabled').html('enabled<div class="tool-tip">Popup is enabled</div>');
          } else {
            $('#layout-item-' + layoutId + ' .layout-item-status').addClass('layout-disabled').html('disabled<div class="tool-tip">Popup is disabled</div>');
          }

          if( isset(ShopifyApp) ) ShopifyApp.Modal.alert("Popup saved");
        } else {
          $('#layout-edit-loader').hide();
...
function initWYSIWYG()
{
  ...
  if( $wysiwygEl.length ) {
    ...
    var editor = new tinymce.Editor($wysiwygEl.attr('id'), {
      ...
      toolbar: "bold,italic,underline,strikethrough,|,forecolor,backcolor,|,alignleft,aligncenter,alignright,alignjustify,|,bullist,numlist,|,formatselect,fontsizeselect,|,ltr,rtl,|,link,unlink,code,|,hr,removeformat,"
